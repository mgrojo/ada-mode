...
(cl-defmethod wisi-xref-parse-final ((xref gnat-xref) _project prj-file-name)
  (setf (gnat-compiler-run-buffer-name xref) (gnat-run-buffer-name prj-file-name gnat-xref-buffer-name-prefix)))
  ...
  (with-no-warnings ;; "unknown slot"
	...
	(let* ((wisi-xref-full-path t)
	  ...
	  (with-current-buffer (gnat-run-buffer prj-xref (gnat-compiler-run-buffer-name prj-xref))
	    ...
	    (goto-char (point-min))
	    (when gnat-debug-run (forward-line 2)); skip ADA_PROJECT_PATH, command
...
(cl-defmethod wisi-xref-other ((_xref gnat-xref) project &key identifier filename line column)
  (let* ((prj-xref (wisi-prj-xref project))
    ...
    (with-current-buffer (gnat-run-buffer prj-xref (gnat-compiler-run-buffer-name prj-xref))
      ...
      (goto-char (point-min))
      (when gnat-debug-run (forward-line 2)); skip ADA_PROJECT_PATH, 'gnat find'
...
(cl-defmethod wisi-xref-parents ((_xref gnat-xref) project &key identifier filename line column)
  (let* ((arg (gnat-xref-common-args project identifier filename line column))
    ...
    (with-current-buffer (gnat-run-buffer project (gnat-compiler-run-buffer-name (wisi-prj-xref project)))
      ...
      (goto-char (point-min))
      (when gnat-debug-run (forward-line 2)); skip GPR_PROJECT_PATH, 'gnat find'
...
(cl-defmethod wisi-xref-all ((_xref gnat-xref) project &key identifier filename line column local-only append)
    ...
    (with-current-buffer (gnat-run-buffer project (gnat-compiler-run-buffer-name (wisi-prj-xref project)))
      (let ((compilation-buffer-name "*gnatfind*")
            ...
            (command-and-args (mapconcat (lambda (a) (or a ""))
                                         (cons (gnat-xref-common-cmd project) arg)
                                         " "))
...
(cl-defmethod wisi-xref-overriding ((_xref gnat-xref) _project &key _identifier _filename _line _column)
  (error "gnat-xref does not support 'show overriding' - use gpr_query?"))

(cl-defmethod wisi-xref-overridden ((_xref gnat-xref) _project &key _identifier _filename _line _column)
