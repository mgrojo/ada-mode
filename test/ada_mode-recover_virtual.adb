...
-- exception handler area, before the first 'when'. This causes the
-- parser to insert several virtual tokens, both there and before the
-- later 'begin end'. Those tokens are then referenced in
-- 'wisi-containing-action' and other grammar actions.
...
--EMACSCMD:(< 0 (length (wisi-parser-parse-errors wisi--parser)))
--EMACSRESULT:t
