...
--
-- Update History:
-- $Log: opentoken-token-analyzer.adb,v $
...
package body Opentoken.Token.Analyzer is
   ...
   -------------------------------------------------------------------------
   -- Increment the given string buffer index by the given amount. If it
   -- ends up outside of the string bounds, it will be wrapped.
   -- To decrement it, suppy a negative number.
   -- The algorithm used relies on the fact that the string buffer will
   -- be ranged 1..Max_String_Length.
   -------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------
   -- Return True if the given index designates a valid element of the
   ...
   -------------------------------------------------------------------------
   -- Get more text for the analyzer's buffer.
   -- The actual amount retrieved depends on the amount available from the
   -- text feeder and either the amount of space left in the buffer, or the
   ...
   -------------------------------------------------------------------------
   -- Returns the number of characters found after the last EOL in the
   -- next Length characters after the buffer's head.
   -------------------------------------------------------------------------
   ...
   begin
      ...
      -- Find the last EOL character in the first Length characters of the
      -- buffer
      if Slice_Tail < Analyzer.Buffer_Head then
   ...
   begin
      ...
      Check_For_Unrecognized : loop
         ...
         for Token_Index in Terminal_Id loop
            Recognizer.Clear (Analyzer.Token_List(Token_Index).Recognizer.all);
         end loop;
         ...
         Check_For_Match : loop
            ...
            for Token_Index in Terminal_Id loop
               ...
               if Match(Token_Index) /= Recognizer.Failed then
                  ...
                  Recognizer.Analyze
                    (The_Token => Analyzer.Token_List(Token_Index).Recognizer.all,
               ...
               case Match(Token_Index) is
                  ...
                  when Recognizer.So_Far_So_Good =>
                     Possible_Matches := Analyzer.Buffer (Current_Char) /= EOF_Character;
      ...
      while More_Possible_Matches loop
         ...
         for Token_Index in Terminal_Id loop
            ...
            if Match(Token_Index) /= Recognizer.Failed then
               ...
               -- keep checking.
               if Match(Token_Index) /= Recognizer.Failed then
                  More_Possible_Matches := Analyzer.Buffer(Current_Char) /= EOF_Character;
               end if;
            end if;
   ...
   begin
      ...
      for Id in Syntax'range loop
         New_Analyzer.Token_List(ID).Token_Handle.ID := ID;
      end loop;
   ...
   begin
      ...
      for Id in Syntax'range loop
         Analyzer.Token_List(ID).Token_Handle.ID := ID;
      end loop;
   end Set_Syntax;
   ...
   begin
      ...
      loop
         ...
         if Matched_Length = 0 then
            if Analyzer.Has_Default then
               ...
               Ada.Exceptions.Raise_Exception(Syntax_Error'Identity, "Unrecognized character '" &
                                              Analyzer.Buffer(Analyzer.Buffer_Head) & "'");
            end if;
         end if;
         ...
         exit when Analyzer.Token_List(Matched_Token).Recognizer.Report;
      ...
      Create (Lexeme     => Lexeme(Analyzer),
              ID         => Matched_Token,
              Recognizer => Analyzer.Token_List(Matched_Token).Recognizer,
              New_Token  => Analyzer.Token_List(Matched_Token).Token_Handle.all
              );
   ...
   begin
      return Analyzer.Token_List (Analyzer.Last_Token).Token_Handle.all;
   end Get;
