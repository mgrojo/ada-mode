...
is
   ...
   Grammar_File_Name_Base       : Ada.Strings.Unbounded.Unbounded_String;
   Need_Gen_Alg_In_Actions_Name : Boolean := False;
   ...
   is
      use Ada.Text_IO;
   begin
      Grammar_File_Name_Base := +Ada.Directories.Base_Name (File_Name);
...
begin
   ...
   declare
      ...
      is begin
         ...
         begin
            ...
            if Output_BNF then
               WisiToken_Grammar_Editing.Print_Source (-Grammar_File_Name_Base & "_bnf.wy", BNF_Tree, Input_Data);
            end if;
      ...
      begin
         ...
         when Meta =>
            if Trace_Generate > Outline then
               Trace.Put_Line ("post-parse main grammar tree META");
            end if;
         ...
         when Other =>
            ...
            if BNF_Tree.Is_Empty then
               if Trace_Generate > Outline then
                  Trace.Put_Line ("post-parse main grammar tree OTHER");
               end if;

               Grammar_Parser.Execute_Actions;
            else
               if Trace_Generate > Outline then
                  Trace.Put_Line ("post-parse bnf grammar tree OTHER");
               end if;
   ...
   begin
      ...
      Need_Gen_Alg_In_Actions_Name := Input_Data.If_Lexer_Present;
      ...
      for Tuple of Generate_Set.all loop
         declare
            ...
            --  do cache Recursions.
            Generate_Data : WisiToken.BNF.Generate_Utils.Generate_Data_Access;
            Packrat_Data  : WisiToken.Generate.Packrat.Data_Access;
         begin
            if Trace_Generate > Outline then
               Trace.New_Line;
            ...
            when Tree_Sitter =>
               ...
               --  Generate_Data.Grammar is built from BNF_Tree; if we are generating
               --  actions, the check in Eliminate_Empty_Productions ensures that is
               --  the same as TS_Tree.
               ...
               begin
                  ...
                  if Trace_Generate > Outline then
                     Trace.Put_Line ("post-parse tree_sitter grammar tree OTHER");
                  end if;
                  ...
                  Eliminate_Empty_Productions (Input_Data, TS_Tree, No_Empty => Tuple.Out_Lang /= None);

                  Print_Tree_Sitter
                    (Input_Data,
                     TS_Tree,
                     TS_Tree.Lexer,
                     Output_File_Name => -Grammar_File_Name_Base & ".js",
            ...
            when LR_Packrat_Generate_Algorithm =>
               ...
               declare
                  ...
                  Parse_Table_File_Name : constant String := -Grammar_File_Name_Base &
                    "_" & To_Lower (Tuple.Gen_Alg'Image) &
                  ...
                  begin
                     ...
                     WisiToken.BNF.Generate_Utils.Put_Stats (Input_Data, Generate_Data.all);
                  ...
                  is begin
                     return Generate_Utils.To_McKenzie_Param (Generate_Data.all, Input_Data.McKenzie_Recover);
                  exception
                  when E : Not_Found =>
               ...
               begin
                  if not Lexer_Done (Input_Data.User_Lexer) then
                     ...
                     when re2c_Lexer =>
                        WisiToken.BNF.Output_Ada_Common.Create_re2c_File
                          (Input_Data, Tuple, Generate_Data.all, -Grammar_File_Name_Base);
                     when others =>
                        null;
                     end case;
                  end if;
                  ...
                  when Packrat_Generate_Algorithm =>
                     ...
                     Packrat_Data := new WisiToken.Generate.Packrat.Data'
                       (WisiToken.Generate.Packrat.Initialize
                     ...
                     declare
                        --  There is no parse_table for packrat, but it's useful to have a
                        --  definitive list of tokens and productions (after all %if are
                        --  processed).
                        Parse_Table_File : File_Type;
                     begin
                     ...
                     if Tuple.Text_Rep then
                        WisiToken.Generate.LR.Put_Text_Rep
                          (Generate_Data.LR_Parse_Table.all,
                           Text_Rep_File_Name (-Grammar_File_Name_Base, Tuple, Input_Data.If_Lexer_Present));
                     end if;
            ...
            when Ada_Lang =>
               ...
               WisiToken.BNF.Output_Ada
                 (Input_Data, Grammar_Parser.Tree.Lexer.File_Name, -Grammar_File_Name_Base, Generate_Data.all,
                  Packrat_Data, Tuple, Test_Main, Multiple_Tuples, Need_Gen_Alg_In_Actions_Name);
            ...
            when Ada_Emacs_Lang =>
               ...
               WisiToken.BNF.Output_Ada_Emacs
                 (Input_Data, Grammar_Parser.Tree.Lexer.File_Name, -Grammar_File_Name_Base, Generate_Data.all,
                  Packrat_Data, Tuple, Test_Main, Multiple_Tuples, Need_Gen_Alg_In_Actions_Name, -Language_Name);
