...
if test $? -eq 0 ; then HAVE_GNAT_XREF="yes"; else HAVE_GNAT_XREF="no"; fi

echo "HAVE_GNAT_XREF=$HAVE_GNAT_XREF"

gnatprep -DHAVE_GNAT_XREF=$HAVE_GNAT_XREF gpr_query-process_refresh.adb.gp gpr_query-process_refresh.adb
gnatprep -DHAVE_GNAT_XREF=$HAVE_GNAT_XREF gpr_query.gpr.gp gpr_query.gpr

gprbuild -p -P gpr_query.gpr
gprinstall -P gpr_query.gpr

gnatprep -DHAVE_GNAT_XREF=$HAVE_GNAT_XREF ada_mode_gps_indent.gpr.gp ada_mode_gps_indent.gpr
