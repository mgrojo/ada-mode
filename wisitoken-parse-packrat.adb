...
package body WisiToken.Parse.Packrat is
   ...
   is
      ...
      begin
         ...
         begin
            User_Data.Reduce (Tree, Node, Tree_Children);

            if Tree.Action (Node) /= null then
               Tree.Action (Node) (User_Data.all, Tree, Node, Tree_Children);
            end if;
         end;
      end Process_Node;
   ...
   begin
      if Trace_Action > Outline then
         Trace.Put_Line ("root node: " & Tree.Image (Tree.Root, Descriptor));
      end if;
