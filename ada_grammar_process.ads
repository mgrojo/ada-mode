--  generated by FastToken Wisi from ada_grammar.wy
...
with FastToken.Token_Wisi_Process;
...
package Ada_Grammar_Process is
   ...
   type Token_Enum_ID is
     (
      ...
      choice_relation_and_list_ID,
      choice_relation_or_list_ID,
      choice_relation_xor_list_ID,
      choice_relation_and_then_list_ID,
      choice_relation_or_else_list_ID,
      choice_relation_ID,
      compilation_unit_ID,
   ...
   function "+" (Item : in Token_Enum_ID) return FastToken.Token_ID
     is (FastToken."+" (FastToken.Token_ID'First, Token_Enum_ID'Pos (Item)));

   function "-" (Item : in FastToken.Token_ID) return Token_Enum_ID
     is (Token_Enum_ID'Val (FastToken."-" (Item, FastToken.Token_ID'First)));

   Descriptor : aliased FastToken.Descriptor :=
     (First_Terminal    => 3,
      Last_Terminal     => 106,
      First_Nonterminal => 107,
   ...
   Trace : aliased FastToken.Text_IO_Trace.Trace (Descriptor'Access);
   State : aliased FastToken.Token_Wisi_Process.State_Type (Trace'Access);
