...
is
   ...
   is begin
      Put_Line
        ("run_ada_parser <verbosity> <enable_mckenzie> <mckenzie_enqueue_limit> <file_name>");
...
begin
   ...
   Parser := Ada_Lite.Create_Parser
     (Algorithm   => WisiToken.LALR,
      Text_Feeder => Feeder);

   if WisiToken.Trace_Parse > 3 then
      ada_lite_dfa.aflex_debug := True;
   end if;
   ...
   if Ada_Lite.State.Errors.Length > 0 then
      New_Line;
      Put_Line ("Errors:");
      WisiToken.Token_Region.Put (To_String (File_Name), Ada_Lite.State.Errors, Ada_Lite.Descriptor);
   end if;
exception
