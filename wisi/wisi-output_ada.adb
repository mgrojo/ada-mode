...
begin
   ...
   Indent_Line ("   Action := (Reduce, Nonterminal.Get (LHS_ID) <= +Synthesize, 0);");
   Indent_Line ("when Accept_It =>");
   Indent_Line ("   Action := (Accept_It, Nonterminal.Get (LHS_ID) <= +Synthesize, 0);");
   ...
   --  FIXME: need count of conflicts; don't output if not needed.
   Indent_Line ("procedure Add_Action");
   Indent_Line ("  (State       : in out LALR_Parsers.Parse_State;");
   Indent_Line ("   Symbol      : in     Token_IDs;");
   Indent_Line ("   State_Index : in     LALR_Parsers.State_Index;");
   ...
   Indent_Line ("   Action_2 : constant Parse_Action_Rec := (Reduce, Nonterminal.Get (LHS_ID) <= +Synthesize, 0);");
   Indent_Line ("begin");
   Indent := Indent + 3;
   ...
   Indent_Line ("   use Productions;");
   Indent_Line
     ("   Action_1 : constant Parse_Action_Rec := (Reduce, Nonterminal.Get (LHS_ID_1) <= +Synthesize_1, 0);");
   Indent_Line
     ("   Action_2 : constant Parse_Action_Rec := (Reduce, Nonterminal.Get (LHS_ID_2) <= +Synthesize_2, 0);");
   Indent_Line ("begin");
   Indent := Indent + 3;
   Indent_Line ("State.Action_List := new Action_Node'");
   Indent_Line
   ...
   for State_Index in Parser.Table'Range loop
      ...
      begin
         loop
            ...
            Set_Col (Indent);
            Put ("Add_Action (Table (" & State_Image (State_Index) & "), " & Token_Name (Node.Symbol));
            declare
