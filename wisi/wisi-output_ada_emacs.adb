...
is
   ...
   procedure Put_Prologue (Comment_Syntax : in String; Prologue : in String_Lists.List)
   is
   begin
      --  Prologue in input file has elisp syntax. If Comment_Syntax = "",
      --  output all (to an elisp source file). Otherwise, only output lines
      --  starting with ";;", replacing those characters with
      --  Comment_Syntax.

      for Line of Prologue loop
         if Comment_Syntax'Length > 0 and (Line'Length = 2 and then Line = ";;") then
            Put_Line (Comment_Syntax);

         elsif Comment_Syntax'Length > 0 and (Line'Length > 2 and then Line (Line'First .. Line'First + 2) = ";; ") then
            Put_Line (Comment_Syntax & "  " & Line (Line'First + 2 .. Line'Last));

         elsif Comment_Syntax'Length > 0 and (Line'Length > 0 and then Line (Line'First) = '(') then
            null;
         else
            Put_Line (Line);
         end if;
