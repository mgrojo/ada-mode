...
(defun wisi-parse-cache-set (parser-state)
  ...
  (with-silent-modifications
    ;; we need a deep copy of parser-state, but we know we only get
    ;; here with these values of some fields.
    (put-text-property
     (point) (1+ (point)) 'wisi-parse-cache
     (make-wisi-parser-state
      :label   0
      :active  'shift
      :stack   (copy-sequence (wisi-parser-state-stack parser-state))
      :sp      (wisi-parser-state-sp parser-state)
      :pending nil))
    ))

(defun wisi-parse (start automaton lexer)
  "Parse current buffer from START using the automaton specified in AUTOMATON.
Parsing resumes from first cache before START.
