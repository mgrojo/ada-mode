...
package OpenToken.Token.Nonterminal is
   ...
   ----------------------------------------------------------------------------
   -- The following primitive routines provide overloadable interfaces for
   -- synthesizing a nonterminal. Child types can provide more of these. Each
   -- will need a function of type Synthesize which calls it.

   -- This exception should get raised when token argument for synthesization
   -- is of the wrong type.
   Invalid_Synth_Argument : exception;

   ----------------------------------------------------------------------------
   -- Synthesization routines for use in creating productions. These routines
   -- are called automaticly by the parser when a production is recognized
   -- in the input (aka: "reduced").
   --

   ----------------------------------------------------------------------------
   -- Synthesize a non-terminal instance from a given list of terminals.
   -- Routines of this type can be called by the parser when it detects a
   -- series of tokens that derive to this token.
   ----------------------------------------------------------------------------
   ...
   -- specified for the production, this routine will be called. It dispatches
   -- to the New_Token instance type's Default_Synthesize routine. For this
   -- instance, that has the same effect as calling Synthesize_First.all.
   -- See the header for that routine for more information.
   --
   -- To change the default behavior in a new token derived from Instance,
   -- override the Default_Synthesize routine.
   --
   -- See the comment on the left-hand-side token's Default_Synthesize
   -- primitive for the list of restrictions for what tokens may legally
   -- appear on the right hand side. If these restrictions are violated, any
   -- exception may be raised, but Invalid_Synth_Argument and Constraint_Error
   -- are most likely.
   ----------------------------------------------------------------------------
   ...
   --
   -- If the first token in the source list is not a nonterminal in the
   -- Instance'Class of token on the left-hand side of the production,
   -- Invalid_Synth_Argument will be raised. Also, you should see the comment
   -- on the left-hand-side token's primitive for the list of restrictions for
   -- what tokens may legally appear in the first position on the production's
   -- right hand side. If these restrictions are violated, any exception may
   -- be raised, but Invalid_Synth_Argument and Constraint_Error are most
   -- likely.
   ----------------------------------------------------------------------------
   Synthesize_First : constant Synthesize;

   ----------------------------------------------------------------------------
   -- Dispatching versions of Token synthesization routines. These routines
   -- may be overridden for types derived from Instance to provide custom
   -- behavior for the visible synthesization procedure pointers.
   --
   -- These routines aren't intended to be be used directly. They are made
   -- public simply so that non-child packages with types derived from Instance
   -- can override them.
   ...
   -- package has the same effect and restrictions as Synthesize_First.
   ----------------------------------------------------------------------------
